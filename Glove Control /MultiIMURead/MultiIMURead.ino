...
#include<SPI.h> 
#include<RF24.h> 
RF24 radio(9, 10) ; //PINS TO SEND ADATA                    
...
void setup() {
  ...
  // setup the raido transmission 
  radio.begin();
  radio.setChannel(0x76) ;  
  radio.setPALevel(RF24_PA_MIN) ;          
  const uint64_t pipe = 0xF0F0F0F0E1LL;//E0E0F1F1E0LL ;  
  radio.openWritingPipe(pipe) ;   // to start writing
...
void LandingCheck(){
    if (findIMUDirection(acc_z[5]) == 1) 
      {
        if( findIMUDirection(acc_z[1]) == 2 && findIMUDirection(acc_z[3]) == 2 && 
            findIMUDirection(acc_z[4]) == 2 )
            {
              Serial.print("Sending Landing Command\n");
//              Transmit("Landing") ; 
